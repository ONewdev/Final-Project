...
function OrdersPending() {
  ...
  // === utils (ให้เหมือนหน้า Orders.jsx) ===
  const getDisplayOrderCode = (o) => {
    if (!o) return '';
    if (o.order_code) return o.order_code; // ถ้ามี order_code จาก backend ใช้เลย
  ...
  const imageSrc = (maybePath) => {
    ...
    if (/^https?:\/\//i.test(str)) return str; // เป็น URL เต็มอยู่แล้ว
  ...
  const getStatusText = (status) => {
    const statusMap = {
      ...
      cancelled: 'ยกเลิก',
      approved: 'ชำระแล้ว/อนุมัติแล้ว', // เผื่อมีใช้ในหน้าเดียวกัน
    };
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (updated.ok) {
          ...
          setOrders(data.filter((o) => o.status === 'pending' || o.status === 'waiting_payment'));
        }
        await Swal.fire({
          icon: 'success',
    ...
    const fetchOrders = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // pending tab แสดง pending + waiting_payment (ถ้าต้องการเฉพาะ pending ให้เหลือ o.status === 'pending')
          setOrders(data.filter((o) => o.status === 'pending' || o.status === 'waiting_payment'));
        } else {
          setOrders([]);
        }
