...
function OrdersProcessing() {
  ...
  // === utils (ให้เหมือน Orders.jsx/OrdersPending) ===
    ...
    const fetchOrders = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setOrders(data.filter((o) => o.status === 'processing'));
        } else {
          setOrders([]);
        }
      } catch {
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="bg-white rounded-lg shadow">
          <div className="p-6 border-b">
            <h2 className="text-xl font-semibold">กำลังเตรียมสินค้า</h2>
          </div>
          {orders.length === 0 ? (
            <div className="p-6 text-center text-gray-500">ยังไม่มีออเดอร์ที่กำลังเตรียมสินค้า</div>
          ) : (
